...
[NOTE.speaker]
The KSecrets API handles secrets grouped into collections. KWallet, on the other hand, gets information about the opening application by the means of the winID given upon openWallet. So perhaps it'll be a good idea to try to use the process name for collection manipulation. And KSecrets API should automatically construct collection name from the calling process name. If doing that, the new API will automatically find secrets written by callin KWallet API.
...
[NOTE.speaker]
When this framework is done, it'll go through kdereview in order to be included into the frameworks tree
